...
class InteractiveSegmentorDataset(PatchDatasetABC):
    def __init__(
        ...
        self.patch_extractor = get_patch_extractor(
            "point",
            input_img=self.img_path,
    ...
    def __getitem__(self, idx):
        ...
        nuc_point = np.zeros(
            (1, self.patch_size[1], self.patch_size[0]), dtype=np.uint8
        )
        nuc_point[
            0, int((self.patch_size[1] - 1) / 2), int((self.patch_size[0] - 1) / 2)
        ] = 1
        ...
        input = np.concatenate(
            (patch, nuc_point, exclusion_map), axis=0, dtype=np.float32
        )  # shape=(c=5,h,w)
    ...
    def get_bounding_box(self, idx):
        ...
        location = (
            self.patch_extractor.locations_df["x"][idx],
            self.patch_extractor.locations_df["y"][idx],
        )
    ...
    def get_exclusion_map(self, idx, bounding_box):
        ...
        sel = xy_locations[
            np.all(
                (xy_locations > bounding_box[:2]) & (xy_locations < bounding_box[2:]),
                axis=1,
            )
        ]

        exclusion_map = np.zeros(
            (1, self.patch_size[1], self.patch_size[0]), dtype=np.uint8
        )
