...
class WSIReader:
    ...
    def open(  # noqa: PLR0911
        ...
        **kwargs: dict,
        ...
        if last_suffix == ".czi":
            return BioFormatsWSIReader(input_path, mpp=mpp, power=power)
...
class BioFormatsWSIReader(WSIReader):
    """Reader for Bio-Formats supported whole-slide images.

    Supported WSI formats depend on Bio-Formats capabilities.

    Attributes:
        bf_reader (:obj:`bioformats.ImageReader`): Bio-Formats reader instance.
    """
    ...
    def read_rect(
        ...
        **kwargs: dict,  # noqa: ARG002
        ...
        """Read a region of the whole slide image at a location and size."""
        # Implement reading logic using Bio-Formats
        if coord_space == "resolution":
            return self._read_rect_at_resolution(
                location,
                size,
    ...
    def _info(self: BioFormatsWSIReader) -> WSIMeta:
        ...
        slide_dimensions = (
            omexml.image(0).Pixels.SizeX,
            omexml.image(0).Pixels.SizeY,
        )
        level_count = omexml.get_image_count() - 2

        level_dimensions = [
            (omexml.image(i).Pixels.SizeX, omexml.image(i).Pixels.SizeY)
            for i in range(level_count)
        ]
        level_downsamples = [
            omexml.image(0).Pixels.SizeX / omexml.image(i).Pixels.SizeX
            for i in range(level_count)
        ]
        vendor = omexml.instrument().Detector.get_Model()

        mpp = (
            omexml.image(0).Pixels.PhysicalSizeX,
            omexml.image(0).Pixels.PhysicalSizeY,
        )
    ...
    def __del__(self):
        """Stop the JVM when the object is deleted."""
        javabridge.kill_vm()


# Make sure to handle JVM lifecycle properly to avoid hanging JVM instances


class BIOWSIReader(WSIReader):
    """Reader for OpenSlide supported whole-slide images.
