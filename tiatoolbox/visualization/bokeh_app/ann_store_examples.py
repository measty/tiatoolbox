...
for i, row in patch_df.iterrows():
    ...
    annotations.append(
        Annotation(
            Polygon.from_bounds(x, y, x + 512, y + 512), 
            {'score': score})
            )
...
centroid_list = [[x1, y1], [x2, y2], ...]
#if its contours each element is a list of points instead
properties_list = [{'score': score1, 'class': class1}, {'score': score2, 'class': class2}, ...]

annotations = []

for annotation, properties in zip(centroid_list, properties_list):
    props = {'score': properties['score'], 'type': properties['class']}
    annotations.append(Annotation(Point(annotation), props))  # use Polygon() instead if its a contour
SQ.append_many(annotations)
...
SQ.patch_many(SQ.keys(), properties_iter=new_props) #replace the properties dict for each annotation
...
query_geom = Polygon.from_bounds(top_left[0], top_left[1], top_left[0] + tile_size, top_left[1] + tile_size)
SQ2 = SQLiteStore()
