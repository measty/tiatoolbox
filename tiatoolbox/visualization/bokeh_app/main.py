...
def change_tiles(layer_name: str = "overlay") -> None:
    ...
    ts = make_ts(
        f"https://{host}:{port}/tileserver/layer/{layer_name}/{UI['user']}/"
    ...
    if layer_name in UI["vstate"].layer_dict:
        ...
        for layer_key in UI["vstate"].layer_dict:
            ...
            grp = tg.get_grp()
            ts = make_ts(
                f"https://{host}:{port}/tileserver/layer/{layer_key}/{UI['user']}/"
                f"zoomify/TileGroup{grp}"
                r"/{z}-{x}-{y}"
                f"@{UI['vstate'].res}x.jpg",
                UI["vstate"].num_zoom_levels,
...
def make_window(vstate: ViewerState) -> dict:  # noqa: PLR0915
    ...
    ts1 = make_ts(
        f"https://{host}:{port}/tileserver/layer/slide/{user}/zoomify/TileGroup1"
        r"/{z}-{x}-{y}"
        f"@{vstate.res}x.jpg",
